...
namespace RobotTools.UI.Editor.Folding
{
    public static class KrlRegularExpressions
    {
        ...
        private static readonly Regex loopLine = new Regex("^\\s*loop(|\\s*|\\s*;.*)$", RO);
        ...
        private static readonly Regex ifLineRegex = new Regex("^\\s*if\\s.*", RO);
        ...
        private static readonly Regex switchRegex = new Regex(@"^\s*switch\s+([^\n]+)$", RO);
        private static readonly Regex endSwitchRegex = new Regex(@"^\s*endswitch([^\s\w]*)$", RO);
        ...
        public static Regex Switch => switchRegex;
        public static Regex EndSwitch => endSwitchRegex;
